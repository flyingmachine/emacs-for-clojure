...
;; Version: 20150127.1216
;; X-Original-Version: 10.3.05
    ...
    (let (curr-char curr-indentation curr-line
      ...
      (cond
       ...
       ((and (get-text-property pos 'block-side)
        ...
        ;; TODO : ne peut on pas se passer des looking-at ?
        (cond
         ;; ((and (string= web-mode-engine "php")
         ;;       (not (web-mode-looking-at "<\\?\\(=\\|php\\)?[ ]*$" reg-beg)))
         ;;  (save-excursion
         ;;    (goto-char reg-beg)
         ;;    (looking-at "<\\?\\(=\\|php\\)?[ ]*")
         ...
         ;;       (not (web-mode-looking-at "{{[{]?[ ]*$" reg-beg)))
         ;;  (save-excursion
         ;;    (goto-char reg-beg)
         ;;    (looking-at "{{[{]?[ ]*")
         ;;    (goto-char (match-end 0))
  ...
  (save-excursion
    (let ((continue t)
      ...
      (while continue
        ...
        ;; (if (or (get-text-property pos 'block-side)
...
(defun web-mode-element-beginning ()
  ...
  (interactive)
  (let ((pos (point)))
    (when (setq pos (web-mode-element-beginning-position pos))
      (goto-char pos))
    pos))
...
(defun web-mode-element-end ()
  ...
  (interactive)
  (let ((pos (point)))
    (when (setq pos (web-mode-element-end-position pos))
      (setq pos (1+ pos))
      (goto-char pos))
    pos))
...
(defun web-mode-part-end ()
  ...
  (interactive)
  (let ((pos (point)))
    (when (setq pos (web-mode-part-end-position pos))
      (when (< pos (point-max))
        (setq pos (1+ pos)))
      (goto-char pos))
    pos))
